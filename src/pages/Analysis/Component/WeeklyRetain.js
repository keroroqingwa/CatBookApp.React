...
// 查询列表数据url
const queryDataUrl = 'analysis/getAnalysisWeeklyRetain';
...
class Donut extends React.Component {
  ...
  render() {
    ...
    if (resByWeeklyRetain && resByWeeklyRetain.columns) {
      for (let i = 0; i < resByWeeklyRetain.columns.length; i += 1) {
        const curCol = resByWeeklyRetain.columns[i]
        for (let j = 0; j < resByWeeklyRetain.data.length; j += 1) {
          const curRow = resByWeeklyRetain.data[j]
          data.push({ country: curCol === 0 ? '当周' : `${curCol}周后`, year: curRow.ref_date, value: curRow.visit_uv[i] || 0 })
          data2.push({ country: curCol === 0 ? '当周' : `${curCol}周后`, year: curRow.ref_date, value: curRow.visit_uv_new[i] || 0 })
        }
      }
    ...
    const tableData = []
    const columns = []
    // console.info('resByWeeklyRetain', resByWeeklyRetain)
    if (resByWeeklyRetain && resByWeeklyRetain.columns) {
      ...
      resByWeeklyRetain.columns.forEach((item, idx) => {
        if (item === 0)
          ...
          columns.push({ title: `${item}周后`, dataIndex: `key_visit_uv_${idx}`, key: `key_visit_uv_${idx}`, width: 80 })
      })

      resByWeeklyRetain.data.forEach((item, idx) => {
        const singleObj = {}
        ...
        <Row gutter={16}>
          <Col span={24}>
            <div className={styles.p20}>
              ...
              <span className={styles.query_data}>当前查询：{resByWeeklyRetain && resByWeeklyRetain.query_data}</span>
            </div>
            <h3>活跃留存</h3>
